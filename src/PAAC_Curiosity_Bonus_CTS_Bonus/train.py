...
from networks.policy_v_network import NaturePolicyVNetwork, NIPSPolicyVNetwork, StateActionPredictor
...
def main(args):
    ...
    network_creator, ICM_network_creator = get_network_and_environment_creator(args)

    if args.learning_algorithm=='paac_cts':
        learner = PAACCTSLearner(network_creator, ICM_network_creator, args)
    else:
        learner = PAACLearner(network_creator, ICM_network_creator, args)
...
def get_network_and_environment_creator(args, random_seed=3):
    ...
    def ICM_network_creator(name ='local_learning_icm'):
        nonlocal network_conf
        copied_network_conf = copy.copy(network_conf)
        copied_network_conf['name'] = name
        return StateActionPredictor(copied_network_conf)
    
    return network_creator, ICM_network_creator


def get_arg_parser():
    parser = argparse.ArgumentParser()
    parser.add_argument('-g', default='doom', help='Name of game', dest='game')
    ...
    parser.add_argument('-ec', '--emulator_counts', default=24, type=int, help="The amount of emulators per agent. Default is 32.", dest="emulator_counts")
    parser.add_argument('-ew', '--emulator_workers', default=6, type=int, help="The amount of emulator workers per agent. Default is 8.", dest="emulator_workers")
